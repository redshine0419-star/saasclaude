    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <Card className="p-6 md:p-8 border-slate-200 bg-gradient-to-br from-white to-slate-50/60">
        <div className="flex items-center gap-3 mb-6">
          <div className="p-3 rounded-xl shadow-lg flex items-center justify-center" style={{background:'linear-gradient(135deg,#4f46e5,#059669)'}}>
        ...
        {anyConnected && session?.user && (
          <div className="space-y-5 border-t border-slate-100 pt-5">
            ...
            {/* GA4 install checker — moved to top */}
            <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl">
              ...
              <div className="flex gap-3">
                <input type="url" placeholder="https://yoursite.com"
                  className="flex-1 pl-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-sm"
                ...
                <button onClick={checkInstall} disabled={checkLoading || !installUrl.trim()}
                  className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 disabled:text-slate-500 text-white font-bold rounded-xl transition-all flex items-center gap-2 whitespace-nowrap text-sm">
                  {checkLoading ? <><Loader2 size={13} className="animate-spin" /> 확인 중...</> : '설치 확인'}
                </button>
              </div>
              {checkResult && (
                <div className={'mt-3 p-3 rounded-xl border text-sm ' + (checkResult.installed ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-rose-50 border-rose-200 text-rose-800')}>
                  <div className="flex items-center gap-2 font-bold">
                    {checkResult.installed ? <CheckCircle2 size={14} /> : <XCircle size={14} />}
            ...
            <button onClick={fetchAll} disabled={loading || !canFetch}
              className="w-full py-4 font-bold rounded-2xl transition-all shadow-lg flex items-center justify-center gap-2 text-base disabled:cursor-not-allowed"
              style={loading || !canFetch ? {background:'#cbd5e1',color:'#64748b'} : {background:'linear-gradient(90deg,#4f46e5,#059669)',color:'#ffffff'}}>
          ...
          <div className="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-2xl">
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-amber-600 mt-1">{t('insight', 'errorHint', lang)}</p>
                <button onClick={fetchAll} className="mt-2 text-xs font-bold text-amber-700 underline hover:text-amber-900">
                  {t('common', 'retry', lang)}
                </button>
              </div>
            </div>
