    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <Card className="p-5 md:p-6 border-rose-100 bg-rose-50/50 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex gap-4 items-center">
          <div className="p-3 bg-rose-500 text-white rounded-2xl animate-bounce">
            <AlertTriangle size={24} />
          </div>
          <div>
            <h4 className="font-bold text-rose-900 text-sm mb-0.5 uppercase tracking-wide">이상 징후 리포트</h4>
            <p className="text-rose-700 text-sm leading-relaxed">
              최근 3시간 내 <strong className="text-rose-900">결제 완료 단계</strong>의 이탈률이 15% 증가했습니다. 모바일 Safari 환경을 점검하세요.
            </p>
          </div>
        </div>
        <button className="whitespace-nowrap px-6 py-2.5 bg-rose-600 text-white text-xs font-bold rounded-xl shadow-lg shadow-rose-100 hover:bg-rose-700 transition-all">
          로그 데이터 즉시 확인
        </button>
      </Card>
        ...
        {[
          ...
          <Card key={i} className="p-5 flex flex-col justify-between h-32">
            ...
            <div className="flex items-end justify-between mt-2">
              ...
              <span className={`text-[11px] font-bold px-1.5 py-0.5 rounded ${m.positive ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                {m.change}
              </span>
            </div>
            <div className="h-1 w-full bg-indigo-500/10 mt-3 rounded-full overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="p-6 md:p-8 flex flex-col">
          ...
          <button className="mt-6 w-full py-3.5 bg-slate-900 text-white rounded-2xl font-bold text-sm hover:scale-[1.02] transition-transform active:scale-95 shadow-lg shadow-slate-200">
            액션 아이템 일괄 처리하기
          </button>
        </Card>
      </div>
