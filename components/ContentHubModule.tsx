...
const Badge = ({ children, variant = 'default' }: { children: React.ReactNode; variant?: string }) => {
  const styles: Record<string, string> = { default: 'bg-slate-100 text-slate-600', primary: 'bg-indigo-100 text-indigo-700' };
  return <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${styles[variant] ?? styles.default}`}>{children}</span>;
};
...
export default function ContentHubModule({ onToast }: { onToast: (msg: string) => void }) {
  ...
  const generateAll = () => {
    ...
    setTimeout(() => { setIsGenerating(false); onToast('모든 채널별 소재 생성이 완료되었습니다.'); }, 2500);
  };
    ...
    <div className="max-w-5xl mx-auto space-y-8">
      <Card className="p-6 md:p-8">
        ...
        <div className="flex flex-wrap gap-2 justify-end">
          ...
          <button onClick={generateAll} disabled={isGenerating || !content}
          ...
          <Card key={i} className="group hover:border-indigo-300 transition-all">
            ...
            <div className="p-6">
              <div className="flex justify-between items-center mb-4">
                ...
                <button className="text-slate-300 group-hover:text-indigo-600 transition-colors"><Copy size={18} /></button>
              </div>
              ...
              <div className="mt-6 pt-4 border-t border-slate-50 flex justify-between items-center">
                ...
                <button className="text-xs font-bold text-indigo-600 flex items-center gap-1 hover:underline">상세 편집 <ChevronRight size={14} /></button>
              </div>
            </div>
          </Card>
        ))}
