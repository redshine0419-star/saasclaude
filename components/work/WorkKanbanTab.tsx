...
import { Plus, Loader2, AlertCircle, ChevronDown, Flag, Calendar, User2, GripVertical, X } from 'lucide-react';
...
export default function WorkKanbanTab({
  ...
  const fetchTasks = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/work/tasks?projectId=${projectId}`);
  ...
  const handleAddTask = async (status: string) => {
    ...
    try {
      const res = await fetch('/api/work/tasks', {
        ...
        body: JSON.stringify({ projectId, title: addTitle.trim(), status }),
      });
      if (res.ok) {
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between px-3 py-3 border-b border-[#d0d7de] dark:border-[#30363d]">
                  ...
                  <button
                    onClick={() => { setShowAdd(col.id); setAddTitle(''); }}
                ...
                <div className="flex-1 p-2 space-y-2">
                  {colTasks.map((task) => (
                    <div
                      ...
                      className={`group bg-white dark:bg-[#0d1117] border border-[#d0d7de] dark:border-[#30363d] rounded-lg p-3 cursor-grab active:cursor-grabbing shadow-sm hover:shadow-md transition-all ${dragging === task.id ? 'opacity-40' : ''}`}
                    >
                      <div className="flex items-start gap-2">
                        ...
                        <button
                          ...
                          className="text-[#8c959f] hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100 shrink-0"
                        >
                          <X size={13} />
                        </button>
                      </div>
                  ...
                  {/* Add task inline */}
                  {showAdd === col.id && (
                    <div className="bg-white dark:bg-[#0d1117] border border-[#0969da] rounded-lg p-2 shadow-sm">
                      <textarea
                        autoFocus
                        value={addTitle}
                        ...
                        onKeyDown={(e) => {
                          ...
                          if (e.key === 'Escape') { setShowAdd(null); setAddTitle(''); }
                        }}
                      ...
                      <div className="flex justify-end gap-2 mt-2">
                        <button onClick={() => { setShowAdd(null); setAddTitle(''); }} className="text-xs text-[#57606a] dark:text-[#8b949e] hover:text-[#24292f] dark:hover:text-[#e6edf3]">취소</button>
                        <button
