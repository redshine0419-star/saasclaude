...
Last updated: 2026-05-11 (Phase 3 업데이트)
...
### AI Share of Voice (AI 언급율) 탭 ✅ 완료

- [x] **`app/api/sov/route.ts`** — Gemini 병렬 호출, 언급 감지, 경쟁사 비교, AI 인사이트
- [x] **`components/SovModule.tsx`** — 원형 게이지, 바 차트, 프롬프트별 아코디언, 측정 이력
- [x] **`lib/storage.ts`** — SovRecord 저장/조회 함수 추가
- [x] **`app/page.tsx`** — 9번째 탭으로 추가 (AI Share of Voice / SOV)

### Phase 2 — 기능 확장

- [x] **리포트 내보내기** — Markdown 다운로드 버튼
- [x] **경쟁사 벤치마크** — CompetitorModule: 2개 URL 병렬 분석·점수 비교
- [x] **GEO 개선 가이드** — llms.txt 자동 생성기 (사이트 정보 입력 → 파일 다운로드)
- [x] **콘텐츠 히스토리** — 생성한 콘텐츠 localStorage 저장·검색
- [x] **키워드 클러스터링** — 여러 키워드 일괄 입력 → 의도별 그룹핑

### Phase 2 — UX 개선

- [x] **다크 모드** — Tailwind dark: 클래스 적용
- [x] **온보딩 플로우** — 첫 방문자 대상 3단계 가이드 모달
- [x] **단축키** — `Cmd+K` 글로벌 커맨드 팔레트 (탭 이동·검색)
- [x] **모바일 최적화** — 콘텐츠 카드 스와이프 제스처
...
- [x] **AI 폴백 레이어** — `lib/ai.ts`: Gemini → Claude 자동 fallback, `FORCE_CLAUDE` 환경변수 지원
- [x] **스케줄 진단** — `vercel.json` Cron: 매주 월 09:00 자동 진단 `/api/cron/weekly-diagnosis`
- [x] **Slack 알림** — `/api/slack/notify`: 점수 ±10점 이상 급변 시 웹훅 알림 + Cron 리포트 전송
